...
import { ArrowLeft, Backpack, CurrencyDollar, PersonSimpleSwim } from "@phosphor-icons/react";
import { rebirthData } from "@/data/rebirthData";
import { rebirthHeroContent } from "@/data/guideContent";
    ...
    <div className="pb-20">
      ...
      >
        <div
          ...
          style={{ backgroundImage: `url(${rebirthHeroContent.backgroundImage})` }}
        />
        ...
        <div className="relative px-6 md:px-12 pt-12 pb-10">
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-blue-500 mb-3">
            {rebirthHeroContent.eyebrow}
          </p>
          <h1
            className="font-heading text-4xl sm:text-5xl font-black tracking-tighter text-white mb-4"
            data-testid="rebirth-title"
          >
            {rebirthHeroContent.title}
          </h1>
          <p className="text-base text-zinc-400 leading-relaxed max-w-xl">
            {rebirthHeroContent.description}
          </p>
        </div>
      </section>
      ...
      <section className="px-6 md:px-12 py-10" data-testid="rebirth-how-section">
        <div className="flex items-center gap-4 mb-6">
          <h2 className="font-heading text-2xl font-bold tracking-tight text-white whitespace-nowrap">
            How Rebirth Works
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 stagger-children">
          {[
            {
              title: "No Full Reset",
              text: "Unlike many games, rebirth does not reset your whole account. It only uses the fish and money required for that rebirth tier.",
            },
            {
              title: "Permanent Perks",
              text: "Each rebirth increases swim speed, backpack space, and your cash multiplier, making every next farm route faster and richer.",
            },
            {
              title: "Plan Ahead",
              text: "Save the required fish before spending or selling them so you can chain rebirths more efficiently when your money is ready.",
            },
          ].map((card) => (
            <div
              key={card.title}
        ...
        >
          {rebirthData.map((rebirth) => (
            ...
            >
              ...
              <div className="mb-3">
                <span className="block text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-1.5">
                  Required Fish
                </span>
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {rebirth.rewards.map((reward) => (
                    <li key={reward} className="text-sm text-zinc-400">
                      {reward}
                    </li>
                  ))}
                </ul>
      ...
      <section className="px-6 md:px-12 py-8" data-testid="rebirth-totals-section">
        ...
        <div className="border border-white/15 rounded-sm p-6 bg-[#0A0A0A]" data-testid="totals-card">
          <p className="text-sm text-zinc-400 mb-5">
            Complete all rebirths to gain these permanent bonuses.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              >
                ...
                <span className="text-2xl font-heading font-black text-white tabular-nums">
                  {stat.value}
                </span>
              </div>
            ))}
