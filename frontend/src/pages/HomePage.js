...
import { fishDatabase, galleryImages, slugify, getFishEntryId, getGroupedZones } from "@/data/fishData";
import { heroContent, quickCards, faqItems, mutationGroups, tipCards } from "@/data/guideContent";
...
const FaqItem = ({ q, a }) => {
  ...
  return (
    <div
      className="border border-white/15 rounded-sm overflow-hidden"
      data-testid={`faq-item-${slugify(q).slice(0, 30)}`}
    >
      <button
      ...
      >
        ...
        {open ? (
          <CaretUp size={16} className="text-blue-500 flex-shrink-0" />
        ) : (
          <CaretDown size={16} className="text-blue-500 flex-shrink-0" />
        )}
      </button>
      {open && (
...
const ZoneCard = ({ zone, fishList }) => {
  ...
  return (
    <section
      id={zoneId}
      className="border border-white/15 rounded-sm overflow-hidden scroll-mt-20"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/20">
              <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-[0.15em] text-zinc-500 bg-black/40">
                Fish Name
              </th>
              <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-[0.15em] text-zinc-500 bg-black/40">
                Rarity
              </th>
              <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-[0.15em] text-zinc-500 bg-black/40 hidden sm:table-cell">
                Notes
              </th>
            </tr>
          </thead>
          <tbody>
            {fishList.map((entry) => (
              ...
              >
                ...
                <td className="px-5 py-2.5">
                  <span
                    className={`inline-block px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider ${getRarityClass(entry.rarity)}`}
                  >
    ...
    <div className="pb-20">
      ...
      >
        ...
        <div className="relative px-6 md:px-12 pt-16 pb-12 max-w-4xl">
          <p
            className="text-[10px] font-bold uppercase tracking-[0.3em] text-blue-500 mb-3"
            data-testid="hero-eyebrow"
          >
            {heroContent.eyebrow}
          </p>
          <h1
            className="font-heading text-4xl sm:text-5xl lg:text-6xl font-black tracking-tighter text-white mb-4"
            data-testid="hero-title"
          >
            {heroContent.title}
          </h1>
          <p
            className="text-base text-zinc-400 leading-relaxed max-w-xl mb-8"
          ...
          {quickCards.map((card) =>
            card.isRoute ? (
              ...
              <button
                ...
                className="group text-left border border-white/15 rounded-sm p-4 hover:border-blue-500 hover:-translate-y-1 transition-all bg-[#0A0A0A]"
                data-testid={`quick-card-${slugify(card.label)}`}
              >
                <card.icon size={24} weight="duotone" className="text-blue-500 mb-2" />
                <p className="text-sm font-semibold text-white group-hover:text-blue-400 transition-colors">
      ...
      <section
        id="database"
        className="px-6 md:px-12 py-8 scroll-mt-20"
        data-testid="database-section"
      >
        <div className="flex items-center gap-4 mb-6">
          <h2 className="font-heading text-2xl sm:text-3xl font-bold tracking-tight text-white whitespace-nowrap">
            Fish Database
          </h2>
        ...
        >
          ...
          <div className="gallery-track flex gap-4 w-max p-4" data-testid="gallery-track">
            {allGallery.map((img, i) => (
              <div
                key={`${img.alt}-${i}`}
                className="flex-shrink-0 w-[36rem] h-52 overflow-hidden border border-white/10 rounded-sm"
              >
        ...
        <section id="mutation-guide" className="mb-10 scroll-mt-20" data-testid="mutation-section">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {mutationGroups.map((group) => (
              <div
                key={group.id}
                className="border border-white/15 rounded-sm p-5 bg-[#0A0A0A]"
                data-testid={`mutation-card-${group.id}`}
              >
                <h4 className="text-xs font-bold uppercase tracking-[0.2em] text-blue-500 mb-4">
                  {group.title}
                </h4>
